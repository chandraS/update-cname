...
def fetch_domain_records(domain_id):
    ...
    if response.status_code == 200:
        ...
        print(f"Failed to fetch DNS records for domain ID: {domain_id}")
        return []
...
def delete_dns_record(domain_id, record_id):
    ...
    if response.status_code == 200:
        ...
        print(f"Failed to delete DNS record ID: {record_id} for domain ID: {domain_id}, Status Code: {response.status_code}")
